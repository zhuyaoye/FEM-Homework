		...
		*this << " NUMBER OF ELEMENTS. . . . . . . . . . .( NPAR(2) ) . . =" << setw(5) << NUME
		...
		switch (ElementType)
		{
			...
			case ElementTypes::Q4: // Bar element
				...
				break;
			case ElementTypes::Q8: // Bar element
...
void COutputter::OutputQ4Elements(unsigned int EleGrp)
{
	...
	*this << "  SET       YOUNG'S     POSSINON		THICK		type" << endl
		...
		*this << " S T R E S S  C A L C U L A T I O N S  F O R  E L E M E N T  G R O U P" << setw(5)
		...
		switch (ElementType)
		{
			...
			case ElementTypes::Q4: // Q4 element
				*this << "  ELEMENT    GAUSS_PT_X        GAUSS_PT_Y        SIGMA_XX        SIGMA_XY        SIGMA_YY" << endl
					<< "  NUMBER" << endl;

				for (unsigned int Ele = 0; Ele < NUME; Ele++)
				{
					CElement& Element = EleGrp[Ele];
					CQ4* q4 = dynamic_cast<CQ4*>(&Element);
					if (!q4) continue;

					double stress[12] = { 0 };
					double gp_coords[8] = { 0 };
					Element.ElementStress(stress, Displacement); // 只算应力
					q4->ElementGauss(gp_coords);                // 只算坐标

					for (int gp = 0; gp < 4; ++gp) {
						*this << setw(5) << Ele + 1
							<< setw(18) << gp_coords[gp * 2 + 0]
							<< setw(18) << gp_coords[gp * 2 + 1]
							<< setw(18) << stress[gp * 3 + 0]
							<< setw(18) << stress[gp * 3 + 2]
							<< setw(18) << stress[gp * 3 + 1]
							<< endl;
					}
				}
			...
			case ElementTypes::Q8: // Q8 element
				*this << "  ELEMENT    GAUSS_PT_X        GAUSS_PT_Y        SIGMA_XX        SIGMA_XY        SIGMA_YY" << endl
				...
				{
					CElement& Element = EleGrp[Ele];
					CQ8* q8 = dynamic_cast<CQ8*>(&Element);
					if (!q8) continue;

					double stress[12] = { 0 };    // 4个高斯点×3分量
					double gp_coords[8] = { 0 };  // 4个高斯点×2坐标

					Element.ElementStress(stress, Displacement); // 计算4个高斯点的应力
					q8->ElementGauss(gp_coords);                // 计算4个高斯点的坐标

					for (int gp = 0; gp < 4; ++gp) {
						*this << setw(5) << Ele + 1
							<< setw(18) << gp_coords[gp * 2 + 0]   // X
							<< setw(18) << gp_coords[gp * 2 + 1]   // Y
							<< setw(18) << stress[gp * 3 + 0]      // SIGMA_XX
							<< setw(18) << stress[gp * 3 + 2]      // SIGMA_XY
							<< setw(18) << stress[gp * 3 + 1]      // SIGMA_YY
							<< endl;
					}
				}
				*this << endl;
			...
			case ElementTypes::T3: // T3 element
				*this << "  ELEMENT             SIGMA_XX            SIGMA_XY            SIGMA_YY" << endl
					<< "  NUMBER" << endl;

				for (unsigned int Ele = 0; Ele < NUME; Ele++)
				{
					CElement& Element = EleGrp[Ele];
					double stress[3] = { 0 }; // 存储应力分量: [SIGMA_XX, SIGMA_YY, SIGMA_XY]
					Element.ElementStress(stress, Displacement);

					// 按格式输出: SIGMA_XX, SIGMA_XY, SIGMA_YY
					*this << setw(5) << Ele + 1
						<< setw(18) << stress[0]   // SIGMA_XX
						<< setw(18) << stress[2]   // SIGMA_XY
						<< setw(18) << stress[1]   // SIGMA_YY
						<< endl;
				}
